...
impl<'a> Drop for PersistTrie<'a> {
    ...
    fn drop(&mut self) {
        ...
        for (block_id, root) in &self.roots {
            // write block_id
            meta_bytes.extend_from_slice(&block_id.to_be_bytes());
            // write root
...
impl<'a> PersistTrie<'a> {
    ...
    pub fn new(db: &'a OptimisticTransactionDB<SingleThreaded>, keep_latest: bool) -> Self {
        let mut roots = BTreeMap::<u32, H256>::new();
        roots.insert(0, H256::default());
        Self {
            roots,
            db,
            block_cnt: 0,
    ...
    pub fn open(db: &'a OptimisticTransactionDB<SingleThreaded>, keep_latest: bool) -> Self {
        ...
        match db.get(meta_key).unwrap() {
            Some(r) => {
                ...
                for i in 0..len {
                    let start_idx = 4 + i * (4 + 32);
                    let block_id = u32::from_be_bytes(r[start_idx .. start_idx + 4].try_into().unwrap());
                    let h = H256::from_slice(&r[start_idx + 4 .. start_idx + 36]);
                    roots.insert(block_id, h);
                }
            },
            None => {}
        }
        let block_cnt = *roots.last_key_value().unwrap().0;
        Self {
            roots,
            db,
            block_cnt,
    ...
    pub fn apply(&mut self, apply: Apply<StateValue>) {
        ...
        if self.keep_latest {
            ...
            self.prune((prune_block_id, prune_block_id)).unwrap();
        }
    }
    ...
    pub fn get_latest_root(&self) -> (u32, H256) {
        let (last_block_id, last_root) = self.roots.last_key_value().unwrap();
        (*last_block_id, *last_root)
    }
    ...
    pub fn print_latest_tree(&self, ) {
        let (_, h) = self.roots.last_key_value().unwrap();
    ...
    pub fn prune(&mut self, block_id_range: (u32, u32)) -> Result<()> {
        let (block_id_left, block_id_right) = block_id_range;
        if block_id_left < *self.roots.first_key_value().unwrap().0 {
            return Err(anyhow!("left block id out of bound"));
        }
        if block_id_right > *self.roots.last_key_value().unwrap().0 {
            return Err(anyhow!("right block id out of bound"));
        }
        for block_id in block_id_left..=block_id_right {
            let root_h = self.roots.remove(&block_id).unwrap();
