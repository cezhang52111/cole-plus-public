...
pub struct ColeIndexBackend<'a> {
    pub nonce_map: BTreeMap<Address, Nonce>,
    pub code_map: BTreeMap<Address, Code>,
    pub states: Cole<'a>,
}

impl<'a> ColeIndexBackend<'a> {
    pub fn new(configs: &'a Configs) -> Self {
        Self {
            nonce_map: BTreeMap::new(), 
            code_map: BTreeMap::new(), 
            states: Cole::new(configs),
        }
    }

    pub fn get_mut_total_tree(&self) -> &'a mut Cole<'a> {
        unsafe {
            let const_ptr = &self.states as *const Cole;
            let mut_ptr = UnsafeCell::new(const_ptr as *mut Cole);
            &mut **mut_ptr.get()
        }
    }
}

impl<'a> Backend for ColeIndexBackend<'a> {
...
impl<'a> BackendWriteTrait for ColeIndexBackend<'a> {
...
mod tests {
    ...
    use cole_index::verify_and_collect_result;
    use utils::compute_cole_size_breakdown;

    #[test]
    fn test_cole_index_backend() {
        let fanout = 5;
