...
impl<K: Num, V: Value> Digestible for POSTreeLeafNode<K, V> {
    fn to_digest(&self) -> H256 {
        // digest of a leaf node is H(H(v_0)||H(v_1)|| ... ||H(v_f))
        let mut bytes = vec![];
        for (_, v) in &self.key_values {
            let v_h = v.to_digest();
...
impl<K: Num + LoadAddr> Digestible for POSTreeInternalNode<K> {
    // digest of an internal node is computed from H(H(h_0) || H(h_1) || ... H(h_{f+1})), assume that there are f+1 child nodes
    fn to_digest(&self) -> H256 {
        let mut bytes = vec![];
