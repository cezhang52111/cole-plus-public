...
use crate::nodes::POSTreeNode;

/* Trait of the POSTree Storage
 */
pub trait POSTreeNodeIO<K: Num + LoadAddr, V: Value> {
    // given a node id, load a node from the storage, if the id does not exist, return none
    fn load_node(&self, node_id: u32) -> Option<POSTreeNode<K, V>>;
    // store a node with node_id
    fn store_node(&mut self, node_id: u32, node: POSTreeNode<K, V>);
    // store a batch of nodes in a map
    fn store_nodes_batch(&mut self, nodes: BTreeMap::<u32, POSTreeNode<K, V>>);
    // remove a node with node_id
    fn remove_node(&mut self, node_id: u32) -> Option<POSTreeNode<K, V>>;
