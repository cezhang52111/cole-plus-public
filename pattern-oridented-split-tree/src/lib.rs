...
fn check_leaf_node_cdc_and_fanout<K: Num, V: Value>(list: &Vec<(K, V)>, exp_fanout: usize, max_fanout: usize) -> Vec<i32> {
    ...
    for index in 0..list_len {
        let r = cdc_hash.generate_cut_point(list[index].1.to_digest().as_bytes());
    ...
    if node_id_vec.is_empty() {
        if entries.entry_num() >= 2 {
            ...
            for i in 1..=cut_points_len - 1 {
                ...
                new_node.next = pointers[i];
                let new_node_id = pointers[i-1];
        ...
        let mut pointers = vec![node_id];
        let mut next_node_id = node.next;
        let mut less_flag = false;
        while index + collect_num_nodes > key_collection.len() {
...
pub fn print_tree<K: Num + LoadAddr, V: Value>(tree: &impl POSTreeNodeIO<K, V>) {
