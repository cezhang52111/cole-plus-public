...
impl<'a> ColePlus<'a> {
    ...
    pub fn insert(&mut self, state: (AddrKey, u32, StateValue)) {
        ...
        pattern_oridented_split_tree::insert(mb_tree_ref, CompoundKey::new(addr_key, ver), value);
    ...
    pub fn check_and_merge(&mut self, ) {
        ...
        while level_id < self.levels.len() {
            if self.levels[level_id].level_reach_capacity(&self.configs) {
                ...
                let new_run = LevelRun::construct_run_by_merge(state_iters, lower_cdc_tree_readers, upper_mht_readers, run_id, next_level_id as u32, &self.configs.dir_name, self.configs.fanout, self.configs.max_num_of_states_in_a_run(next_level_id as u32), next_level_num_of_run, self.configs.size_ratio, &mut self.cache_manager, self.configs.is_pruned);
    ...
    pub fn memory_cost(&self) -> MemCost {
        ...
        let (state_cache_size, model_cache_size, cdc_cache_size, mht_cache_size, offset_cache_size) = self.cache_manager.compute_cacher_size();
        MemCost::new(state_cache_size, model_cache_size, cdc_cache_size + mht_cache_size + offset_cache_size, filter_size)
    }
